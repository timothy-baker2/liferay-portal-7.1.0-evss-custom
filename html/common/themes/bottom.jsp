...
//  so that the browsers request for 'main.js' resolves properly on the Eauth junction.
//  Else, use the default path returned by Liferay.
	StringBuffer jsThemePath = new StringBuffer( themeDisplay.getPathThemeJavaScript() );
	if (request.getServerName() != null
			&& request.getServerName().contains("eauth")
			&& jsThemePath != null
			&& jsThemePath.indexOf("sep-theme") > 0
			&& jsThemePath.indexOf("sep/sep/") == -1) {
		jsThemePath.insert(jsThemePath.indexOf("sep-theme")-1, "/sep");
	}
%>
<script src="<%= HtmlUtil.escape(PortalUtil.getStaticResourceURL(request, jsThemePath.toString() + "/main.js")) %>" type="text/javascript"></script>
